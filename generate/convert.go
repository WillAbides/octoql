...
func (g *generator) convertDefinition(
			...
			if conflict := goNames[goName]; conflict != nil {
				return nil, errorf(val.Position,
					"enum values %s and %s have conflicting Go name %s; "+
						...
						"to 'casing' in octoql.yaml to fix",
	...
	case ast.Scalar:
		...
		return nil, errorf(
			pos, "unknown scalar %v: please add it to \"bindings\" in octoql.yaml"+
				"\nExample: https://github.com/willabides/octoql/blob/main/example/octoql.yaml", def.Name)
	default:
