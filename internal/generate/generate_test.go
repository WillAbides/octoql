	...
	for _, file := range files {
		...
		t.Run(testFilename, func(t *testing.T) {
			...
			case "ConflictingTypeNames.graphql":
				snaps.MatchInlineSnapshot(t, err.Error(), snaps.Inline("testdata/errors/ConflictingTypeNames.schema.graphql:2: conflicting definition for the Go type T: it is generated from both the selection of GraphQL type T (at testdata/errors/ConflictingTypeNames.schema.graphql:2) and the selection of GraphQL type T (at testdata/errors/ConflictingTypeNames.schema.graphql:2), which select different fields (expected 2 fields, got 1); give one of them a distinct name with an @octoqlgen(typename:) directive so they produce separate Go types"))
