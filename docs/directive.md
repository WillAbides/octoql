...
The `@octoqlgen` quasi-directive configures octoqlgen on a
query-by-query basis.

Its syntax is just like a GraphQL directive, except that it goes in a comment
on the line immediately preceding the node it applies to. This is because
...
applies to all relevant nodes within it. Every other directive applies to all
nodes on the following line. In all cases other comments may appear between the
directive and the nodes it applies to.
...
query, `d` applies to `arg2` and `arg3`, `e` applies to `field1` and `field2`,
and `f` applies to `field3`.
...
With great power comes great responsibility: when using `typename` you must
avoid conflicts. octoqlgen complains if the same type name is used in multiple
